export async function onRequest(context) {
  ...
  if (url.pathname === '/signal') {
    ...
    server.accept();
    const rooms = new Map(), roomMetas = new Map(), allClients = new Set();
    ...
    function genCode() { const c='ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; let r; do { r=''; for(let i=0;i<5;i++) r+=c[Math.random()*c.length|0]; } while(rooms.has(r)); return r; }
    const info = { curRoom: null, curRole: null };
    allClients.add(server);
    send(server, { type: 'room_list', rooms: [] });
