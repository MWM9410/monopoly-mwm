// Durable Object - 信令服务器状态（跨 Workers 隔离共享）
...
export class SignalRoom {
  constructor(state, env) {
    ...
    this.allSockets = new Map(); // ws -> {currentRoom, currentRole}
  }
  ...
  async fetch(request) {
    ...
    this.allSockets.set(server, info);
    ...
    server.addEventListener('message', (event) => {
      ...
      switch (msg.type) {
        ...
        default:
          send(server, { type: 'error', message: 'unknown_type' });
      }
    });

    server.addEventListener('close', () => {
      this.allClients.delete(server);
      this.allSockets.delete(server);
